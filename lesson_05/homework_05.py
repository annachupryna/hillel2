...
# Крок 1: Фільтрація автомобілів за критеріями
filtered_cars = []
for car_name, details in car_data.items():
    # Перевіряємо, чи відповідає автомобіль критеріям
    year = details[1]
    engine_volume = details[2]
    price = details[4]
    if year >= search_criteria[0] and engine_volume >= search_criteria[1] and price <= search_criteria[2]:
        filtered_cars.append((car_name, details))

# Крок 2: Сортування за ціною
for i in range(len(filtered_cars)):
    for j in range(i + 1, len(filtered_cars)):
        if filtered_cars[i][1][4] > filtered_cars[j][1][4]:  # Порівнюємо за ціною
            # Міняємо місцями, якщо поточний елемент дорожчий за наступний
            filtered_cars[i], filtered_cars[j] = filtered_cars[j], filtered_cars[i]

# Крок 3: Виведення перших п'яти результатів
for car in filtered_cars[:5]:
    name = car[0]
    details = car[1]
    print(f"{name}. Characteristics - Color: {details[0]}, Year: {details[1]}, Engine Volume: {details[2]}, Type: {details[3]}, Price: {details[4]}")
